...
defmodule Nerves.WSL do
  ...
  def get_wsl_paths(file, _use_wslpath) do
    ...
    win_path =
      if Regex.match?(~r/(\/mnt\/\w{1})\//, full_path) do
        # extract drive letter from path
        %{"drive" => drive_letter} = Regex.named_captures(~r/\/mnt\/(?<drive>\w{1})\//, full_path)
        # replace /mnt/<drive_letter>/ with windows version C:/
        win_path =
          Regex.replace(~r/(\/mnt\/\w{1})\//, full_path, "#{String.upcase(drive_letter)}:/")

        # replace forward slashes with backslashes
        Regex.replace(~r/\//, win_path, "\\\\")
      else
        # If path is already a windows path, return it
        if valid_windows_path?(full_path) do
          full_path
        else
          nil
        end
    ...
    wsl_path =
      if valid_windows_path?(full_path) do
        ...
        %{"drive" => drive_letter} = Regex.named_captures(~r/(?<drive>^.{1}):/, full_path)
        # replace <drive_letter>: with /mnt/<drive_letter>
        wsl_path = Regex.replace(~r/^.{1}:/, full_path, "/mnt/#{String.downcase(drive_letter)}")
