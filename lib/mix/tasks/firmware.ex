...
defmodule Mix.Tasks.Firmware do
  ...
  defp build_firmware(config, system_path, fw_out) do
    ...
    post_processing_script = post_processing_script_arg(firmware_config)
  ...
  defp post_processing_script_arg(firmware_config) do
    case {verity_options(firmware_config[:verity]), firmware_config[:post_processing_script]} do
      {nil, nil} ->
        []

      {nil, user_script} when is_binary(user_script) ->
        ["-s", user_script]

      {verity_opts, user_script} when is_nil(user_script) or is_binary(user_script) ->
        ["-s", write_verity_script(verity_opts, user_script)]
    end
  end

  defp verity_options(nil), do: nil
  defp verity_options(false), do: nil
  defp verity_options(true), do: []

  defp verity_options(opts) when is_list(opts) do
    opts = Enum.map(opts, &expand_verity_path/1)

    case Nerves.Verity.validate_options(opts) do
      :ok ->
        opts

      {:error, reason} ->
        Mix.raise("Invalid Mix config for :nerves, :firmware, :verity: #{reason}")
    end
  end

  defp verity_options(other) do
    Mix.raise("""
    Unexpected Mix config for :nerves, :firmware, :verity. It should either be
    `true` or a list of dm-verity options. See `Nerves.Verity` for the options.

    Got: #{inspect(other)}
    """)
  end

  defp expand_verity_path({key, path})
       when key in [:signing_key, :signing_cert] and is_binary(path) do
    {key, Path.expand(path)}
  end

  defp expand_verity_path(other), do: other

  # Write a rootfs post-processing script that runs Nerves.Verity on the
  # SquashFS after it has been built. If the user configured their own
  # post-processing script, it runs first since the dm-verity metadata has to
  # be computed last.
  defp write_verity_script(verity_opts, user_script) do
    script_path = Path.join([Mix.Project.build_path(), "nerves", "verity_post_process.sh"])
    File.mkdir_p!(Path.dirname(script_path))

    elixir =
      System.find_executable("elixir") ||
        Mix.raise("Could not find the elixir executable in $PATH")

    user_script_line = if user_script, do: ~s["#{user_script}" "$1"\n], else: ""

    exports =
      Enum.map_join(verity_opts, fn {key, value} ->
        name = "NERVES_VERITY_" <> String.upcase(Atom.to_string(key))
        ~s[export #{name}="#{value}"\n]
      end)

    File.write!(script_path, """
    #!/bin/sh
    # Generated by mix firmware. Do not edit.
    set -e
    #{user_script_line}export NERVES_VERITY_ROOTFS="$1"
    #{exports}exec "#{elixir}" -pa "#{Application.app_dir(:nerves, "ebin")}" -e "Nerves.Verity.main()"
    """)
