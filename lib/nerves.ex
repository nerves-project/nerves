...
defmodule Nerves do
  ...
  defp artifact_source_files(config, dep) do
    ...
    #
    # Nerves 2.x uses `:artifact_source_files`
    ...
    file_patterns =
      config[:nerves][:artifact_source_files] || config[:nerves_package][:checksum] ||
    ...
    if missing_paths != [] do
      raise Nerves.InvalidPlan,
        ...
        For Nerves 2 packages, update the `:nerves` configuration's `:artifact_source_files`.
